                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <tr v-if="blog.length" v-for="blogs in blog" v-bind:key="blogs.id">
                                        <td>{{ blogs.id }}</td>
                                        <td><img class="table-image" :src="'./upload/project/' + blogs.images"></td>
                                        <td>{{ blogs.title }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <router-link :task="'123123'" :to="{ path: '/project/edit', query: { edit: blogs.id} }" tag="button" class="btn btn-primary">Chỉnh sửa</router-link>
    ...
    methods : {
        ...
        updateds_status(id) {
          ...
          swal({
            ...
            if (result.value) {
                axios.post('/api/project/update_status',{'id': id}).then(response => {
                  this.getdataindex();
        ...
        deleteCategory(id,images) {
                let $this = this;
                  swal({
                    ...
                    if (result.value)
                    {
                      axios.post('/api/project/delete', {'id': id, 'images': images}).then(response => {
                        $this.getdataindex();
