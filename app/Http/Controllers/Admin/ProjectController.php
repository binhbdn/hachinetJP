...
class ProjectController extends Controller
{
    ...
    public function store(Request $request){
            ...
            $check = DB::table('project')->insert($config);
            if($check == true)
            {
    ...
    {
        $id = $request->id;
        $blog = DB::table('project')->where('id',$id)->first();
    ...
    public function update(Request $request){
            ...
            $check = DB::table('project')->where('id', $id)->update($config);
            if($check == true)
            {
    ...
    public function update_status(Request $request){
        ...
        $check_status = DB::table('project')->where('id',$request->id)->select('*')->first();
        ...
        $check = DB::table('project')->where('id',$request->id)->update($input);
        if ($check == true) {
    ...
    public function delete(Request $request){
        ...
        if($blog_images !== null){
            $blog_images = base_path().'/public/upload/project/'.$blog_images;
            if(file_exists($blog_images)){
                unlink($blog_images);
            }
        }
        
        $check = DB::table('project')->where('id', $id)->delete();
        if($check == true)
        {
